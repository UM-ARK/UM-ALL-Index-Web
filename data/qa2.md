...
- **賬號名 - 自定義**。在登入 ARK ALL 時輸入，支持英文、數字。
- **登錄密碼 - 自定義**。在登入 ARK ALL 時輸入，支持英文、數字。
...
- 發件人姓名。
  ...
  請發件人使用澳大電郵(`@um.edu.mo` 或 `@connect.um.edu.mo` 或 `@umac.mo`)將以上註冊信息發送到 `umacark@gmail.com`。發件人的姓名和 UM ID 將被記錄。如有冒名、違反用戶協議等行為，則依法追究。
...
### 6. Q: 什麼是ARK？
A: ARK的中文是方舟，起源於Game - ARK 方舟生存進化，也可以起源於科技學院 - 電機系。每天都似乎在末日中生存，面對如此多Report、Assignment、Exam的我們，何不團結在一齊？像是搭上同一艘方舟，一齊生存。這就是ARK。
...
A: 只要將問題發送到我們的 email 即可，我們會盡快回覆。
